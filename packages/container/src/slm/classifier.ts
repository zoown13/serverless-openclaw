...
function promptFor(input: SlmClassificationInput): string {
  return [
    ...
    "For active payment context, use activeContext facts such as lastQueryMode, topicKeywords, paymentRecordCount, candidateCount, scanLimit, and lastSearchQuery to infer whether the user wants grouping, narrower filtering, broader coverage, a date rerun, or general chat handoff.",
    "If active payment context already has topicKeywords and the user asks for more of that same topic, keep sourceChoice gmail and choose rerun_current_task or refine_current_task, not generic chat.",
    "If the user says variants of \"그거 말고\", \"다른 질문\", or \"일반 질문\" and then asks for recommendations, commands, explanations, coding, translation, or casual chat, choose switch_to_chat even if an active Gmail/payment context exists.",
    ...
    "Example: active payment context + \"그거 말고 일반 질문인데 저녁 메뉴 추천해줘\" => {\"action\":\"switch_to_chat\",\"taskFamily\":\"generic_tool_task\",\"sourceChoice\":\"general\",\"confidence\":0.95}.",
    ...
    "Example: active payment context + \"일본 것만\" => {\"action\":\"refine_current_task\",\"taskFamily\":\"gmail_payment_summary\",\"sourceChoice\":\"gmail\",\"followUpIntent\":\"refine_topic\",\"confidence\":0.95}.",
    "Example: active payment context + \"지난주로 다시 봐줘\" => {\"action\":\"refine_current_task\",\"taskFamily\":\"gmail_payment_summary\",\"sourceChoice\":\"gmail\",\"followUpIntent\":\"refine_date\",\"confidence\":0.95}.",
    ...
    "Example: active payment context + \"더 있을텐데\" => {\"action\":\"rerun_current_task\",\"taskFamily\":\"gmail_payment_summary\",\"sourceChoice\":\"gmail\",\"followUpIntent\":\"coverage_check\",\"confidence\":0.95}.",
    "Example: active payment context + \"빠진 거 없어?\" => {\"action\":\"rerun_current_task\",\"taskFamily\":\"gmail_payment_summary\",\"sourceChoice\":\"gmail\",\"followUpIntent\":\"coverage_check\",\"confidence\":0.95}.",
    "Example: active payment context + \"전부 다시 봐줘\" => {\"action\":\"rerun_current_task\",\"taskFamily\":\"gmail_payment_summary\",\"sourceChoice\":\"gmail\",\"followUpIntent\":\"coverage_check\",\"confidence\":0.95}.",
    ...
    `activeContext: ${JSON.stringify({
      status: input.activeStatus ?? null,
      ...
      lastSearchQuery: truncate(input.activeLastSearchQuery),
      topicKeywords: input.activeTopicKeywords ?? [],
      lastQueryMode: input.activeLastQueryMode ?? null,
      paymentRecordCount: input.activePaymentRecordCount ?? null,
      candidateCount: input.activeLastCandidateCount ?? null,
      scanLimit: input.activeLastScanLimit ?? null,
      resultEstimate: input.activeLastResultEstimate ?? null,
