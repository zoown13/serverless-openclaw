...
const FINANCE_LOOKUP_PATTERN =
  /(?:결제|지출|사용금액|사용 금액|승인내역|카드값|카드\s*(?:사용|결제)|여행\s*경비|출장\s*경비|청구서|영수증|명세서|\bpayment(?:s)?\b|\bcharge(?:s|d)?\b|\btransaction(?:s)?\b|\bspent\b|\bspend\b|\bbilling\b|\binvoice\b|\breceipt\b|\bstatement\b)/i;
...
const PAYMENT_RECORD_PATTERN =
  /(?:결제한?\s*내역(?:들)?|지출\s*내역(?:들)?|사용\s*내역(?:들)?|여행\s*경비|출장\s*경비|카드사별|결제처별|\brecords?\b|\bhistory\b)/i;
